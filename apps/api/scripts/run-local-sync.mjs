...
async function main() {
  ...
  const login = await request("POST", "/api/auth/login", { email: "admin@ecommerce.com", password: "AdminSecure123!" });
  console.log("login", login.data?.user?.role);
  const cat = await request("POST", "/api/admin/sync/categories");
  console.log("categories", cat.data);
  const brand = await request("POST", "/api/admin/sync/brands");
